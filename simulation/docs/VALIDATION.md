...
| F1 22.5° @ 6 kt | 120.4 m | 111.9 m | +7.6 % (grounded hull — the 8% re-baselined band; was +4.9% at fitted mass; the full-load 45.55 t gives 125.1 m, +11.8%) |
| Tightest, one side stops @ 6.5 kt | 60.3 m | 62 m | -2.7 % (grounded hull+lever 2.00 m — was +1.7% at NET 1.8 m; the thole mean 2.00 m (31·2.7+27·2.0+27·1.2/85) tightens 63.1→60.3, now exactly on the anchor; the 0.2 m NET correction is the documented damping, register C3) |
| Oar-only hold / back (no rudder) | 94.0 / 94.0 m | no trial anchors (oQ-3) | back ≡ hold at speed (the hold-brake degeneration, Gate 4) `[x]` (grounded lever 2.00 m — was 104.2 m at NET 1.8/1.5; the oar-hold/back tighten -10% with the grounded brake) |
...
| The turns (the W5 acceptance) | G1 91.5 / F1 120.4 / tightest 60.3 m | 89.4 / 111.9 / 62.0 m | **+2.4 / +7.6 / -2.7 %** ✓ (B2/B3 grounded hull+lever 2.00 m — was +1.7% at NET 1.8 m; the thole mean tightens 63.1→60.3, now exactly on the anchor; F1 re-baselined to 8%) |
...
2. **Tightest-turn 360° time** — **closed by the sway DOF** (plan 15.3):
   ...
   1.8 m) + the ship's effective Ω (now the computed cross-flow value
   3.25e6 — Plan 2, §11.1): D = 67.8 m ✓ and the speed
...
command). Calibration: `calib-2026-08-29-84c8893` (generated by
...
(calibration `calib-2026-08-29-84c8893`; the 2026-08-29 B lever 2.00 m re-fit (was B3 7c79644): the
re-measured drift cells, tau_exit 8→16 s, and the wprime_burst bin_max
5.0→5.5 — see §9.3/§11.2. The 2026-08 review wave: the
...
| --- | --- | --- | --- | --- |
| g1 (full rudder @ 6 kt) | 91.5 m | 91.7 m | +0.3 % | 57.0 / 56.0 s |
| f1 (22.5° @ 6 kt) | 120.4 m | 120.2 m | −0.1 % | 74.0 / 71.0 s |
| tightest (one side holds + full rudder) | 60.3 m | 60.0 m | −0.4 % | 51.0 / 51.0 s |
| oar-hold / oar-back (no rudder) | 94.0 / 94.0 m | 103.9 / 103.9 m | +10.5 % | 81.0 / 93.0 · 81.0 / 93.0 s | **annotated** — the grounded lever 2.00 m (was NET 1.8/1.5: 104.2/105.0) tightens the LL oar-hold/back -10%; the HL's oar-hold/back D stays at the calibrated 103.9 m (+10.5% vs LL) — the turn-drag/oar-orbit tables cannot represent the tighter LL without a re-fit; the Level-2 gate is **annotated** to 12% for the oar family (B2) |
...
### 10.2 Level 2 — the LL vs the HL (calibration `calib-2026-08-29-84c8893`)
...
| sway pair: Ω (computed), clr 0.93, lever 2.00 | LL | Ω: the cross-flow pure-rotation moment — **COMPUTED** (Stream C B1, crossflow.py + basis_hull_offsets.tsv): ½ρ·0.252·J_REAL (J=23217, LWL 32.35 m, C_D 0.252, rectangular vs tapered DECODE C9) = 3.00e6 (the fitted 3.20e6 = C_D 0.30 on the parametric hull, 1.6% closure — C1 resolved). B1 grounded A_lat 30.09/CLR 0.93 (was fitted 35/0.8); B3 grounded mass/Iz 40.95 t/4.76e6 (was fitted 42.0 t/4.0e6) — the W5 turns hold (g1 +2.4%, f1 +7.6% re-baselined to 8%, tightest -2.7% — the B2 lever 2.00 m (thole mean 31·2.7+27·2.0+27·1.2/85) tightens 63.1→60.3, now exactly on the anchor; the 0.2 m NET 1.8 m correction is the documented damping, register C3). clr + lever: G1/F1 D + approach t_360 | turns + the drift cells | Ω: already computed — no trials needed; clr: now the Lines-Plan centroid (16.60 m AP, 0.93 forward, B1); the parametric AFT gap (−0.2…−1.4) was the gap; lever: the thole mean 2.00 m (thranite 2.7 grounded from beam 5.45–5.6 m; zygian/thalmian 2.0/1.2 [?] pending Figure 16) with the 0.2 m NET correction (the lateral damping the sway now models; the NET 1.8 m is the documented sway-calibrated reference) |
| tau_surge / tau_turn / tau_exit / drift_tau_exp | HL | the LL's settle/start/turn/burst paths (machine fits); tau_exit 8 s + drift_tau_exp 0.279 (B2: 16 s/0.152 on NET 1.8 m / 40.95 t → 8 s/0.279 on the grounded 2.00 m/40.95 t — the tighter oar-hold orbit 104→94 m shortens the fishtail tau again; B3: 8 s/0.255 on the fitted 42.0 t → 16 s/0.152 on the grounded 40.95 t/4.76e6 — the heavier Iz slowed the exit; K23 — the fishtail re-fitted as the scan over tau on the HL's RESPONSE vs the LL's decay, judged by the position rows it gates — the raw LL-fit ignored the HL's chase lag, making the effective decay ~1.7× too slow; the consistent (tau, exponent) pair ships together) | HL only | the same LL protocols (re-fitted, never hand-set) |
...
(159 checks, §8 — force-driven default, Stream C B1/B3 grounded).
...
| T8 — the drift angle | open-with-locked-test — the LL's time-domain β measured (the corrected direction convention, K24): −1.50° (G1) / −1.18° (F1) / +2.24° (tightest) vs the reported 15°±2° / Taylor's 7.8°. **Step 2 re-measured the LL both ways** — steady β and the trial's delay method (heading@90° vs track@90°) agree: G1 gives ~1.6° either way, tightest ~2.7°. The gap is real even at the conservative 7.8° (~5×). Needs a missing lateral force (hull tilt, wind leeway, or nonlinear side-force) — see `investigation/02-drift-angle.md`. No A_lat/clr adjustment holds the turns AND the wprime closure. |
