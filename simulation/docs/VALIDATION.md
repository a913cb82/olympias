...
command). Calibration: `calib-2026-08-16-6c96ffc` (generated by
...
(calibration `calib-2026-08-16-6c96ffc`; the 2026-08 review wave: the
...
| --- | --- | --- | --- | --- | --- |
| long_cruise (20 min, steady) | +0.0 % | −0.021 pts | — | 0.063 NM | PASS |
| sprint_turn (25 min, 2 turns + bursts) | +0.7 % | −0.005 pts | — | 0.280 NM | **annotated** — the turn-phase composition at the d-scaled cells + the fishtail's tau_exit pair (K29's re-scan closed the zig-zag at the sprint's cost; §11.2) |
| wprime_burst (30 min, 2 bursts + recovery) | −0.1 % | −0.005 pts | — | 0.079 NM | PASS — the sway-transient closure holds |
| cruise_turn.txt (the sample script) | −1.9 % | −0.142 pts | — | 0.063 NM ✓ | **annotated** on the mean/fatigue only — the position row CLOSED by the K28 mixed-hold fix (0.194 → 0.063; §11.2) |
| three_nm_cruise (35 min, the 3-NM gate) | +0.0 % | −0.005 pts | −0.0 % | 0.063 NM | PASS — the t_3nm gate's first number |
| tempo_loss (exhausted sprint) | −0.1 % | −0.013 pts | — | 0.007 NM | PASS |
| zig-zag (out-of-sample, task T10) | +1.3 % | −0.000 pts | — | 0.010 NM ✓ | **annotated** on the mean only — the position row CLOSED by the K29 re-scan (0.186 → 0.010, clean); the reversal-mix mean +1.3 % stays (§11.2) |
...
### 10.2 Level 2 — the LL vs the HL (calibration `calib-2026-08-16-6c96ffc`)
...
refill cycles), the sprint_turn's position 0.280 (the fishtail's
tau_exit pair's cost — the K29 re-scan's trade, §11.2). All five turns
PASS (±1.3 %; the tightest 62.5/62.5 — the +9.2 % row closed); the
t180's inside the ±20 % band (the worst row −6 % — the K29's τ_hold
re-measurement halved the old −12 %).
