...
command). Calibration: `calib-2026-08-16-77f4258` (generated by
...
(calibration `calib-2026-08-16-77f4258`; the 2026-08 review wave: the
...
| sprint_turn (25 min, 2 turns + bursts) | +0.7 % | −0.005 pts | — | 0.199 NM | **annotated** — the turn-phase composition residual at the drained-state turns (the yaw-build/drift/fishtail interplay; §11.2) |
...
| zig-zag (out-of-sample, task T10) | +1.3 % | −0.000 pts | — | 0.300 NM | **annotated** — the fishtail-reversal mix's composition (the T10 finding; §11.2) |
...
### 10.2 Level 2 — the LL vs the HL (calibration `calib-2026-08-16-77f4258`)
...
| Position after course changes | clean on long/wprime/three_nm/tempo (0.007–0.043 NM); **annotated** on the turn-mixed scripts (sprint_turn 0.199, cruise_turn 0.086 ✓, zig-zag 0.300 — the turn-phase composition residual: the yaw-build/drift/fishtail interplay at the drained-state and reversal mixes, §11.2; the named triggers: the per-state drift, the oscillatory fishtail). The drift cells are dt-sensitive (the validation dt pinned — T7) | §9.3.4 + §11.2 | — |
...
| T10 — the zig-zag (out-of-sample) | landed + annotated — found the steady-turns' deceleration (fixed by T4) and the reversal-mix's composition residual: the mean +1.3 %, the position 0.300 NM |
...
refill cycles), the sprint_turn's position 0.199 and the zig-zag's
