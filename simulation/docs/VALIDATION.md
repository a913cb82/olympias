...
| sway pair: Ω (computed), clr 0.8, lever 1.8 | LL | Ω: the cross-flow pure-rotation moment — **COMPUTED** (Plan 2, crossflow.py): ½ρ·0.3·J over the parametric hull + the ram, the drag-crisis C_D (literature); the audit's closure: the fitted 3.2e6 equals the computation at 1.6 % and the turn gates hold (g1 +0.9 %, f1 +5.6 %, tightest +1.0 % — no regression) — the C1 units caveat resolves (Ω IS the quadratic cross-flow yaw moment). clr + lever: G1/F1 D + approach t_360 | turns + the drift cells | Ω: already computed — no trials needed; clr: the computed lateral-plane centroid is AFT of the c.g. (−0.2…−1.4 m vs the fitted +0.8 forward — the parametric ends + the ram don't reproduce it) — the real lines (Wolfson Plan 7 / the Eliav CAD) are the named path; lever: the A1 decomposition |
