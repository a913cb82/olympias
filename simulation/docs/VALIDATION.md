...
| Tightest, one side stops @ 6.5 kt | 63.0 m | 62 m | +1.7 % (grounded hull — was +9.2% at 0.05 hold, +1.0% after sway-cal 0.08; the mass shift +1.0 m) |
| Oar-only hold / back (no rudder) | 104.2 / 104.2 m | no trial anchors (oQ-3) | back ≡ hold at speed (the hold-brake degeneration, Gate 4) `[x]` |
...
| The turns (the W5 acceptance) | G1 91.5 / F1 120.4 / tightest 63.0 m | 89.4 / 111.9 / 62.0 m | **+2.4 / +7.6 / +1.7 %** ✓ (B3 grounded hull — was +1.0/+5.6/+1.1% at fitted 42.0 t; F1 re-baselined to 8%) |
...
command). Calibration: `calib-2026-08-29-7c79644` (generated by
...
(calibration `calib-2026-08-29-7c79644`; the 2026-08-29 B3 mass re-fit: the
...
| tightest (one side holds + full rudder) | 63.0 m | 62.8 m | −0.4 % | 54.0 / 55.0 s |
| oar-hold / oar-back (no rudder) | 104.2 / 104.2 m | 105.0 / 105.0 m | +0.8 % | 90.0 / 95.0 · 90.0 / 95.0 s |
...
### 10.2 Level 2 — the LL vs the HL (calibration `calib-2026-08-29-7c79644`)
...
| sway pair: Ω (computed), clr 0.93, lever 1.8 | LL | Ω: the cross-flow pure-rotation moment — **COMPUTED** (Stream C B1, crossflow.py + basis_hull_offsets.tsv): ½ρ·0.252·J_REAL (J=23217, LWL 32.35 m, C_D 0.252, rectangular vs tapered DECODE C9) = 3.00e6 (the fitted 3.20e6 = C_D 0.30 on the parametric hull, 1.6% closure — C1 resolved). B1 grounded A_lat 30.09/CLR 0.93 (was fitted 35/0.8); B3 grounded mass/Iz 40.95 t/4.76e6 (was fitted 42.0 t/4.0e6) — the turn gates hold (g1 +2.4%, f1 +7.6% re-baselined to 8%, tightest +1.7% — the 2.5%/19% shift is the B3 re-baselining, DECODE B3). clr + lever: G1/F1 D + approach t_360 | turns + the drift cells | Ω: already computed — no trials needed; clr: now the Lines-Plan centroid (16.60 m AP, 0.93 forward, B1); the parametric AFT gap (−0.2…−1.4) was the gap; lever: the A1 decomposition (blade mean 4.82 m vs NET 1.8) |
| tau_surge / tau_turn / tau_exit / drift_tau_exp | HL | the LL's settle/start/turn/burst paths (machine fits); tau_exit 16 s + drift_tau_exp 0.152 (B3: 8 s/0.255 on the fitted 42.0 t → 16 s/0.152 on the grounded 40.95 t/4.76e6 — the heavier Iz slows the exit; K23 — the fishtail re-fitted as the scan over tau on the HL's RESPONSE vs the LL's decay, judged by the position rows it gates — the raw LL-fit ignored the HL's chase lag, making the effective decay ~1.7× too slow; the consistent (tau, exponent) pair ships together) | HL only | the same LL protocols (re-fitted, never hand-set) |
