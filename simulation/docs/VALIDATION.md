...
command). Calibration: `calib-2026-08-16-f006dc5` (generated by
...
(calibration `calib-2026-08-16-f006dc5`; the 2026-08 review wave: the
...
| sprint_turn (25 min, 2 turns + bursts) | +0.7 % | −0.005 pts | — | 0.201 NM | **annotated** — the turn-phase composition residual at the drained-state turns (the yaw-build/drift/fishtail interplay; §11.2) |
...
| zig-zag (out-of-sample, task T10) | +1.3 % | −0.000 pts | — | 0.318 NM | **annotated** — the fishtail-reversal mix's composition (the T10 finding; §11.2) |
...
7. **The oar-back settled orbit** `[x]` — the one-side-back turn's D at
   ...
   *drained spiral* — while W′ lasts the LL holds V ≈ 3.5–4 kt and
   ...
   as the speed falls, so D shrinks to 17–45 m. The fatigue part is
   CLOSED (the K21 follow-up): the HL's one-side-stopped legs now drain
   at the measured FRESH nets (net_fresh — the turns' full-tank entry,
   ~the symmetric pull at the low rates) while the tank is full, the
   drained ~0 nets only after — the turn scenarios' depletion sits at
   −0.004/−0.005 vs the LL (the 0.05 gate), and the drained-entry legs
   (the cruise_turn context, K13) keep the ~0 nets unchanged. The
   remaining boundary is the orbit: the HL models the oar turn as a
   fixed-diameter orbit (d_oar, measured at the half-circle) at its
   settled speed — by design it cannot follow a speed-dependent spiral,
   and a D(V) alternative would break the half-circle gate (the HL's V
   at the half-circle is 2.1 kt, not the LL's 3.7 — its back-mode drag
   lands on the drained equilibrium). The hold mode settles cleanly in
   the LL (oar-hold: 1.09×). Named trigger: a scenario that demands the
   drained-orbit fidelity (then the back mode needs a W′/V-dependent
   orbit, not a table cell). Locked: `SETTLED_D_RATIO` +
   the per-turn depletion gate in harness/tests/test_equivalence_gates.py.
...
### 10.2 Level 2 — the LL vs the HL (calibration `calib-2026-08-16-f006dc5`)
...
| Position after course changes | clean on long/wprime/three_nm/tempo (0.007–0.043 NM); **annotated** on the turn-mixed scripts (sprint_turn 0.201, cruise_turn 0.086 ✓, zig-zag 0.318 — the turn-phase composition residual: the yaw-build/drift/fishtail interplay at the drained-state and reversal mixes, §11.2; the named triggers: the per-state drift, the oscillatory fishtail). The drift cells are dt-sensitive (the validation dt pinned — T7) | §9.3.4 + §11.2 | — |
...
| the yaw-build two-timescale | HL | the LL's yaw approach (helm A 0.60/tf 6.0/ts 10.0; oar A 0.95/tf 3.0/ts 10.0 — the sway-coupled slow mode) | HL only | re-fitted from the LL's ω(t) |
| the asym nets + net_fresh | HL | the one-side-stopped legs' W′ drain: the drained-state nets ≈ 0 (the settled orbit, K13) AND the fresh-phase nets (the turns' full-tank entry: spoude 32.8/47.6/63.0/64.7/59.9 at 25.5/28.8/32.3/36/44.5 — measured, K21; back ≡ hold, the degeneration) | HL only | re-measured per protocol change |
...
| T3 — the turn timing | the t180 row is gated at ±20 % (the measured timing-loose band: the HL systematically fast in the D-matched turns, the worst rows −17 %/+16 %) |
...
| T10 — the zig-zag (out-of-sample) | landed + annotated — found the steady-turns' deceleration (fixed by T4) and the reversal-mix's composition residual: the mean +1.3 %, the position 0.318 NM |
...
refill cycles), the sprint_turn's position 0.201 and the zig-zag's
composition (+1.3 % / 0.318 — the turn-phase composition residual: the
yaw-build/drift/fishtail interplay). All five turns PASS (±3.9 %; the
tightest 62.7/61.7 — the +9.2 % row closed); the t180's inside the
±20 % band.

The K20/K21 follow-up (the replay UI's oar-back view): the settled orbit
...
1.30 clean / 4.00 annotated). The fatigue divergence that the UI
surfaced is CLOSED (K21): the HL's one-side-stopped legs now use the
measured fresh nets (net_fresh) while the tank is full — the turn
scenarios' depletion delta −0.004/−0.005 (the 0.05 gate, locked in the
same test); the drained-entry legs (the K13 cruise_turn context) keep
the ~0 drained nets, unchanged.
...
to ~20–45 m. The fatigue part is CLOSED (K21): the fast sim's
one-side-stopped legs now drain at the measured fresh nets while the
tank is full (its rowing side empties in ~80 s, matching the LL's
~90 s — the turn scenarios' depletion delta −0.005), and the drained
nets (~0) apply only after. What stays: its oar-turn model is a single
fixed diameter, so it holds its one calibrated ~100 m circle forever
(even drained, its power is already the low-speed value and the chase
floor sits above the drained targets — the orbit cannot follow the
spiral; the W′/V-dependent orbit is the named trigger). Documented and
locked (the settled-window ratio: 2.56× measured vs a 4.0× bound; the
other four turns sit at 1.03–1.09×).
