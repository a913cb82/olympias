...
| 9 | the hull law | the power the hull needs at each speed — the resistance curve | tank-tested on the 1:10 lines-plan model (Grekoussis & Loukakis) + trial-validated (ch.7/ch.9) — a measurement of the lines, like the offsets, not a sea-trial fit | the hull's resistance curve | **PROMOTED (goal work): the LL now sails the computed law** — ITTC-1957 friction (WSA 130.5) + k·V⁴ wave (k = 5.3). Measured switch cost: cruise equilibria ±0.06%, G1/F1 +0.0/+0.01 m, 30-s burst −0.002 kt (F1 band 8.5→8.6% for the +8 mm, posterior rule). The trial-WL WSA variant (~122) evaluated and NOT promoted (F1 lands exactly on the band edge — no headroom). Tank law stays as the reference | tank law: measured (lines evidence, carries); LL uses computed (2) |
