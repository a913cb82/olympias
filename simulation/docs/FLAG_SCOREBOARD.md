...
|---|---|---|---|
| `stations` → `True` | Per-station positions/flows | Helm +43/+117%, hold −12% (mode-independent; rotation-specific, translation agrees 2%; ablation: sweep is the whole yaw balance, r-terms only −10/−14%) | Sweep-oscillatory-blade data (dynamic stall / wake-return / free-surface — CFD/measurements; k/lcp/omega over-constrained + Gate-1); wake-rotation minor; polar-partial (~−10%, Figure-10); slip unfittable |
