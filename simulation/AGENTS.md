...
- `ll/` (low-level) is the oracle — per-oar physics, validated against the
  research numbers.
- `hl/` (high-level, Phase 2 — plan §19) is a fast approximation; its response curves
...
locks the research side itself). Per-gate counts live in VALIDATION.md. Parser has
...
- [x] Phase 1 Gate 3 — 170-oar surge+yaw ship (`ll/ship.py` + `rig.py`):
      time-domain turns reproduce the W5 anchors within 5 % —
