...
command). Calibration: `calib-2026-08-15-b55e28f` (generated by
`hl/calibrate.py`, ~2.5 min of LL protocols, LL commit b55e28f; the
residuals live in the calibration file; the loop calibrate → validate →
adjust ran three rounds — each round found and fixed a real measurement
protocol bug, see §9.3).

### 9.1 The script set (plan §20) — Level-2 first tolerances

| script | mean speed | fatigue consumed | distance | position sep | verdicts |
| --- | --- | --- | --- | --- | --- |
| long_cruise (20 min, steady) | +0.0 % | −0.021 pts | +0.0 % | 0.499 NM | all gates PASS except the drift floor `[x]` |
| sprint_turn (25 min, 2 turns + bursts) | +1.5 % | −0.005 pts | +1.5 % | 1.214 NM | the turn-deceleration residual `[!]` + drift floor `[x]` |
| wprime_burst (30 min, 2 bursts + recovery) | +0.5 % | −0.005 pts | +0.5 % | 1.918 NM | all gates PASS except the drift floor `[x]` |
| cruise_turn.txt (the sample script) | +1.2 % | −0.005 pts | +1.2 % | 0.866 NM | the back-tail transition residual `[!]` + drift floor `[x]` |
...
| ch.7 cruise triple 25.5/32.3 (Mark II hull refs) | marginal (−1.6 % / −5.3 % at hull = 1.0; printed reference-only) | §2 | run the LL at hull = 1.08 (the Mark II drag multiplier) vs 7.0/7.5/8.0 kt — validates or documents the known tension (plan §21.2.5) |
...
| 360° turn time | failed — 98 vs 128 s, open, no known cause | §7.2 | test the linear yaw-damping hypothesis (register C1); close or lock as the sole open physics item (plan §21.2.6) |
...
| Mark IIb equivalence, cant, sway (Gates 6–8) | validated as equivalences | §6/§7 | the Mark IIb blade law ((q/p)² turning point) is the next physics layer — implement and re-validate (plan §21.2.7) |
| Cross-cutting chain checks (couple, P = 7.43·r origin, R&W, F/G ≤ 7 %) | validated | §6 | — |

### 10.2 Level 2 — the LL vs the HL (calibration `calib-2026-08-15-b55e28f`)
...
| Fatigue consumption, all 4 scripts | validated (−0.005 pts) | §9.1 | — |
| Mean speed, turn-heavy scripts (cruise_turn, sprint_turn) | failed (+1.2 % / +1.5 % vs the 1 % gate) | §9.3.5 | the back-tail per-state τ + the measured turn-deceleration term (plan §21.2.3/4) |
| Position after course changes | failed vs 0.1 NM — the drift floor ~0.017 NM/min (the LL's untrimmed lateral kick) | §9.3.4 | decision point: the gate re-expressed as |sep| − drift floor, or a calibrated bias-yaw (plan §21.3) |
| Time to 3 NM | never exercised — no script crosses 3 NM (the longest covers 2.82 NM) | §6 | a ~40-min cruise script; the comparator already computes it (plan §21.2.2) |
| Settled stroke rate within 1 spm | not implemented in the comparator (the HL's rate_eff = commanded always) | §6 | implement the metric + a measured tempo-loss curve for the HL (plan §21.2.1) |
