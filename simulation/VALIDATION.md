...
command). Calibration: `calib-2026-08-15-c243c01` (generated by
...
| 360° turn time | open (locked test) — 98.2 vs 128 s; the linear yaw-damping hypothesis tested and FAILED (task H): any closing k blows G1/F1/tightest out of their bands (+24/+31/+25 %); `YAW_LIN_DAMP` OFF with the negative result recorded. The 128 s is trial-derived ('halves speed' → mean 2.91 kt → 2.81°/s) — the LL's turn-speed loss (−32 %) is the remaining target (T2); the turn build-up (~2 s) and the yaw/oar differential (~1 s) are ruled out (plan §17/§18) | §7.2 + §11.2 | open — the cause quantified (the turn-speed floor: the LL's ~3.2 vs the trial's implied ~2.9 kt); a measured mechanism, or stays open |
...
### 10.2 Level 2 — the LL vs the HL (calibration `calib-2026-08-15-c243c01`)
...
| Numeric pressures / helm fractions between the anchors | interpolated, not gated — the sweep grid closed this: the interpolation midpoints measured + gated at the standard tolerances (the LL's steady curve is non-monotone — the 30-spm dip) | §18 + §11.2 | the gates are defined at the schema anchors; the interpolation residuals are recorded, not gated (scope decision, plan §21.3) |
| Waves, per-side pressure steering, exhausted-side yaw, tempo loss, Mark IIb rig, old-fir fleet, reduced crews, rates < 8 spm | scoped (Phase 4/5 or HL-loose with named triggers) | §18 | re-opened only when a scenario demands it (plan §21.3) |
...
(M1–M8) and the task wave (T1–T10, the DAG's second half — plan §21.2)
...
| the asym nets | HL | the one-side-stopped legs' W′ drain (≈ 0 vs the symmetric 68 W/man — the settled-orbit state) | HL only | re-measured per protocol change |
