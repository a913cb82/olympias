...
def run_crew(name: str, vkt: float, n_cycles: int = 6,
             pressure: str = "chain") -> tuple:
...
def test_drive_times():
    for name, vkt in POINTS:
        oar, plan, _, V = run_crew(name, vkt)
    ...
    print(f"       force-driven drive times (chain demand): "
...
def test_thrust_7_2():
    oar, plan, steps, V = run_crew("Olympias", 7.2)
    ref = rigid_stroke(V, RIGS["Olympias"], SPM["Olympias"][7.2],
    ...
    print(f"       emerging mean thrust at 7.2: {fx_mean:.2f} N/oar vs "
...
def test_flip_entry():
    for name, vkt in POINTS:
        ...
        w_eq_catch = (V * math.cos(math.radians(rig["sweep"]) / 2) * cf
                      + vn_eq) / l_cp
        ...
        vn_mean = sum(abs(s.vn) * DT for s in _last_drive_steps(name, vkt)) \
            / max(T_DRIVE[(name, vkt)], 1e-9)
...
def test_work_conservation():
    for name, vkt in POINTS:
        ...
        w_handle = sum(s.Fh * abs(s.omega) * oar.rig["lin"] * DT
                       for s in steps if s.immersed)
...
def test_start_from_rest():
    ...
    deadspot's ~2 kN."""
    r = SPM["Olympias"][7.2]
    crew = TierCrew("Olympias", 1, r, T_DRIVE[("Olympias", 7.2)],
    ...
    print(f"       first drive from rest: {oar.t_drive_last:.3f} s "
