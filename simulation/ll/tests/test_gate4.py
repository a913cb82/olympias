"""Gate 4 — the rower physiology layer (the LL gates — docs/VALIDATION.md).

Run: python3 tests/test_gate4.py  (from simulation/)

Gates (the LL gates — docs/VALIDATION.md):
...
def test_back_hold():
    ...
    s = loop(
        Ship(rate=rt, oar_state=("row", "back"), helm=("midship", 0.0)), 20, v0_kt=6.5
    )
...
def test_tightest_long():
    ...
    assert s.V / KT < 0.85 * vmax / KT, (
        f"V(900) {s.V / KT:.2f} vs peak {vmax / KT:.2f} kt"
    )
