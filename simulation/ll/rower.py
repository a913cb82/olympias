...
class TierCrew:
    ...
    def __init__(
        ...
        self._side = side
        self._rigs = (
            [stations_short_rig(rig) if st[2] else rig for st in stations]
            if stations
            else None
        )
    ...
    def _fh_moments(
        ...
        if backing:  # vn = V cosC + l_cp w  (blade forward)
            v2_mean = (
                V * V * cos2_mean
                + 2.0 * V * self.l_cp * omega * cos_mean
    ...
    def _omega_for_mean(
        self, V: float, sweep: float, fh_target: float, backing: bool = False
    ) -> float:
    ...
    def _held_stations(self, brake: float) -> list:
        ...
        for o, rg, st in self._pairs:
            if st is not None:
                x_b, y_b = blade_pos(
                    st[0], st[1], self._side, rg["lout"], self._side * C
                )
            else:
                x_b = y_b = 0.0
    ...
    # ------------------------------------------------------------------
    def _plan_force(
        self, V: float, B: float, lin: float, l_cp: float, k: float
    ) -> StrokePlan:
        ...
        if self.mit > 0.0:
            ...
            if fh_flip > self.Fh_max:
                fh_flip = self.Fh_max
                w_entry = max(0.01, fh_flip * lin * self.t_rise / self.mit - w_rec)
    ...
    def plan_stroke(self, V: float) -> StrokePlan:
        ...
        if backing:
            ...
            if self.W <= 0.0:
                ...
                fh_avail = min(
                    fh_avail,
                    self.P_crit * self.power_factor * 60.0 / (B * lin * self.rate_cmd),
                )
    ...
    def step(
        ...
        if self.plan is None or (
            (self.oar.in_drive or self.oar.in_flip)
            ...
            if self.force:
                ...
                for o in self.oars:
                    o.configure_stroke(
                        self.plan.omega, self.plan.omega_recover, self.plan.sweep
                    )
        ...
        if self.plan.limited_by == "back-hold":
            ...
            self.p_gross_current = 0.0
            v_check = (
                math.sqrt(self.Fh_max * self.lin / (self.k * self.l_cp))
                - 0.05 * self.l_cp
            )
            if V < v_check:
        ...
        if self._vgeo is not None and not self.force and ship_state is not None:
            ...
            for o, rg, st in self._pairs:
                ...
                if st is not None:
                    x_b, y_b = blade_pos(
                        st[0], st[1], self._side, rg["lout"], self._side * o.C
                    )
                else:
                    x_b = y_b = 0.0
                ...
                fh += s.Fh
            self._stations = out
        self.p_gross_current = (
            self.plan.p_ext * scale
    ...
    def _stations_step(
        self, dt: float, V: float, ship_state: tuple, scale: float
    ) -> tuple[float, float, float]:
        ...
        return (float((Fx * scale).sum()), float((Fy * scale).sum()), float(Fh.sum()))
    ...
    def end_of_step(self, dt: float) -> None:
        ...
        if p > p_crit_g:
            ...
            self.W = min(
                self.W_max, self.W + min(p_crit_g - p, self.W_max / self.tau) * dt
            )
...
class SideCrew:
    ...
    def __init__(
        ...
        if fleet == "spruce":
            ...
            mit = {
                "thranite": OAR_TIER_MIT["thranite"],
