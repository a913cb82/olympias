...
def main():
    ...
    print(f"sprint protocol (44.5 spm spoude, W' fade, one side holds, "
          f"full rudder); anchors: D = {D_ANCHOR:.0f} m, t_360 = {T360_ANCHOR:.0f} s")
    print(f"{'hold_frac':>9} {'D m':>7} {'t_360 s':>8} {'V_360 kt':>9} "
          f"{'score':>7}")
    best = None
    for i in range(args.steps + 1):
        f = args.f_min + (args.f_max - args.f_min) * i / args.steps
        ship = Ship(rate=44.5, oar_state=("row", "hold"),
                    helm=("starboard", 1.0), hold_frac=f)
        ship.V = 6.5 * KT
