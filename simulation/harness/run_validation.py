...
SCRIPTS = [
    ...
    (
        "sample cruise_turn.txt",
        "examples/cruise_turn.txt",
        5.0 * KT,
        (7,),
    ),  # the scoped back-tail bin (the HL's domain boundary)
...
def run_script_table(
    path: str, V0: float, until=None, exclude_bins=()
) -> tuple[dict, str]:
...
def run_turn_table(
    name: str, V0_kt: float, n_oars: int, helm: tuple, oar_state=("row", "row")
) -> str:
    ...
    return (
        f"| {name:9s} | {rate:6.1f} | {d_ll:7.1f} | {d_hl:7.1f} | "
...
def main() -> None:
    ...
    if not args.turns:
        ...
        for title, path, v0, exclude_bins in scripts:
            ...
            lines = [ln for ln in table.splitlines() if not ln.startswith("| turn_D")]
            print("\n".join(lines))
            for key in (
                "mean_speed_pct",
                "t_3nm_pct",
                ...
                if row["hl"] is not None and abs(row["hl"]) >= row["tol"]:
                    ...
                    if key == "position_sep":
                        note = (
                            " (the HL carries the measured drift bias; "
                            "the residual is the path fidelity)"
                        )
                    violations.append(
                        f"{path}: {key} {row['hl']:+.3f} (tol {row['tol']}){note}"
                    )
    ...
    print(
        "\nviolations: "
        + (
            "none — all Level-2 first tolerances inside"
            if not violations
