...
def run_both(commands, until=None, V0=0.0, ll_dt=LL_DT, hl_dt=HL_DT,
             sample=SAMPLE_S) -> dict:
    ...
    t_end = until if until is not None \
        else (commands[-1].time if commands else 0.0)
    return dict(
        ll=rows_ll, hl=rows_hl,
        meta=dict(
            ll_dt=ll_dt, hl_dt=hl_dt, sample=sample, V0=V0, t_end=t_end,
...
def turn_stream(rate: float, helm: tuple, oar_state=("row", "row")) -> list:
    ...
    cmds = [Command(time=0.0, verb="rate", args=[rate], lineno=1),
            Command(time=0.0, verb="helm", args=[helm[0], float(helm[1])],
                    lineno=2)]
    if oar_state[0] != "row" or oar_state[1] != "row":
        ...
        cmds.append(Command(time=0.0, verb="oars", args=[state, side],
                            lineno=3))
    return cmds
