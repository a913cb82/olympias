# Carter's wave-prediction equations (reproduction of Shaw's Table 8.3 inputs)

Lane 2 ("wave environment"), Olympias trireme reconstruction — research notes.
Primary source verified against the full text of the original paper (17-page PDF obtained from the
WaveLab reference archive, USGS GitLab). All confidence flags: [x] = certain (verified in primary source and/or multiple independent secondary sources), [?] = uncertain.

---

## 1. Identity of "Carter's equations"
...
- NOT the SMB (Sverdrup–Munk–Bretschneider) method per se — but Carter *compares* his fetch-limited heights with
  Bretschneider's SMB formula and finds "good agreement" (see §6). The SMB/Bretschneider (1973) and
  Darbyshire (1963) formulas are alternatives/checks, not what "Carter" means. [x]

---

## 2. The equations (Carter 1982, verbatim from original paper, Appendix A & Eqs 15–18)
...
### 2a. Fetch-limited case (use when D > criterion of eq. 18)  [x]
```
...
### 2c. Which regime applies (fetch-limited iff):  [x]
```
...
### 2d. Fully developed sea (saturation) limits  [x]
...
### 2e. Sea/swell boundary (eq. 19)  [x]
Growing sea (not swell) requires:
```
(a) X ≥ 2.32 · U^2     (fetch-limited regime)
(b) D ≥ 2.01 · U       (duration-limited regime)
```
(equivalently the dimensionless peak frequency ν = U·f_m/g ≥ 0.14). Beyond these, use §2d caps.

### 2f. Underlying dimensionless forms (Table 1, col. 6–7, Hasselmann 1976 parametric values)  [x]
...
```
(Table row for dimensionless height is OCR-garbled in the PDF; coefficients shown match §2a/§2b exactly,
which are the authoritative published forms.) [? on exact Table-1 notation, [x] on the resulting equations]

### 2g. Equivalence of fetch and duration (constant wind)  [x]
...
### 2h. Spectrum-integrated (non-recommended) forms — do NOT use for prediction  [x]
...
## 3. Period-definition ambiguity — critical for reproducing Table 8.3
...
Table 8.3 cell-by-cell from the duration-limited equations (15b/17b) with the fully-developed cap (§2d):
all 36 cells of H, L, C match the decoded table to the printed precision. See
`research/data/shaw-table-8.3-significant-waves.csv` and the verification note in
`research/lane-2-waves/carter-equations.md` §10. (Carter text ch.8 defines T as "the time that
elapses between successive occasions on which the sea surface rises above its mean level" — that is
precisely the zero-up-crossing period.) The T_m variant would give L and C larger by 1.66× and 1.29×
respectively, which does not match the table.

---

## 4. Deep-water dispersion: L = 1.56·T² and C = 1.56·T  [x]
...
- Group velocity (deep water): C_g = C/2 = 0.78·T — this is the speed at which wave *energy* travels and
  underlies the fetch/duration equivalence (§2g). [x]

---

## 5. Valid ranges and validity notes  [x]
...
## 6. Comparison formulas (for cross-checking Shaw's numbers)
...
## 7. Example numbers (Aegean-like, fetch-limited, steady wind)
...
9. [x] **Tucker, M. J. & Pitt, E. G. (2001).** *Waves in Ocean Engineering.* Elsevier — cites H_s ≈ 0.024·U² for FDS
   (Carter 1982 attribution), consistent with §2d.

---

## 9. Open questions / items for the orchestrator
...
## 10. Table 8.3 reconstruction & verification (Shaw 2012 p.72)
...
and the duration-limited cap H_s = 0.0240·U² — all 36 cells verified, see §10.**
