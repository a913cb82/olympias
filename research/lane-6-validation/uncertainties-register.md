...
| C3 | Physical decomposition of the oar-race lever (Table 31.1 row 10: lever_oar 4.8 m Olympias / 5.4 m Mark IIb) | fitted, decomposition open | `[?]` | Taylor ch.31 (fitted to trial turns) | The pure-surge athwartships arms are only ~1.5–2 m (outrigger offsets), so the 4.8 m likely folds in stopped-blade drag and drift dynamics. The LL uses the fitted lever as the validated torque scale (Gate 3, rig.py); per-oar stations (Coates Plan 8) would allow decomposition. | [M] oar-only turn tightness; the fitted value reproduces the trial turns ≤5 % |
...
| D7 | Anaerobic capacity W′ per rower | **5 kJ/man in use** (ch.9 four-run sprint anchor); upper envelope ~9.5 kJ (¾-NM run) | `[x]` anchor / `[?]` model tension | ch.9: four-run sprint sustained 8.2–8.3 kt ~45 s at 44.5 spm (sim excess 116.6 W/man × 45 s ≈ 5.2 kJ); ch.4: "2-min power ≈ 1.2× 6-min" rule (→ ~3.2 kJ at P_crit 80 W); ¾-NM 6.5-min calibration run at 104.3 W/man (→ ~9.5 kJ) | The 2-parameter critical-power model cannot fit all three anchors (a 3-parameter model would); the sim's 45-s sprint sustain matches the trials, so 5 kJ is used and the ¾-NM long-sprint tension is documented. | [M] sprint/fade timing |
