...
REF_CHARS = ("0123456789"
             ...
             "\u00c5\u03a9\u03c9\u03a6\u03c6\u03b8\u03b2\u03c1\u03bb\u03a8"
             "Wwm\u221a\u2248\u2265\u2264\u2192\u2022\u00a7\u00b6")
REF_FONTS = ["/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf",
...
def main():
    ...
    for line in sys.stdin.read().splitlines():
        ...
        if line.startswith('<') and '>' in line:
            try:
                a, b = line.split()
                reverse.setdefault(b.strip('<>').lstrip('0').upper() or '0', a.strip('<>').lstrip('0').upper() or '0')
            except Exception:
                pass
    f = TTFont(ttf)
    gset = f.getGlyphOrder()
    refmap = build_refmap()
    print('n glyphs in font:', f['maxp'].numGlyphs)
    # For each gid present (per reverse mapping we map PUA->CID)
    for pua in sorted(reverse, key=lambda x: int(x or '0', 16)):
        cid_hex = reverse[pua]
        gid = int(cid_hex, 16)
        if gid >= len(gset):
            print(f"F{pua.zfill(4)} -> GID {gid} OUT OF RANGE"); continue
        img = glyph_bitmap(f, gid)
        u = norm(img)
        if u is None:
            print(f"F{pua.zfill(4)} (gid {gid}): EMPTY"); continue
        best = sorted(((dist(u, r), c) for c, r in refmap.items()), key=lambda t: t[0])[:4]
        print(f"F{pua.zfill(4)} (gid {gid}): " + ", ".join(f"{c!r}({d})" for d, c in best))
