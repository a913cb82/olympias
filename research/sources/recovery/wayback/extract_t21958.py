...
def extract_articles(html_str):
    articles = []
    pattern = re.compile(
        r"<article\s+id=['\"]elComment_(\d+)['\"].*?</article>", re.DOTALL
    )
...
def get_author(seg):
    m = re.search(
        r"cAuthorPane_author.*?ipsType_break['\"][^>]*>(?:<span[^>]*>)?([^<]+?)(?:</span>)?</a>",
        seg,
        re.DOTALL,
    )
    if not m:
        m = re.search(
            r"cAuthorPane_author.*?>(?:<strong>)?<a[^>]*>([^<]+)</a>", seg, re.DOTALL
        )
...
def get_edited(seg):
    m = re.search(
        r"Edited\s*<time datetime='([^']+)'[^>]*>(.*?)</time>\s*by\s*([^<]+)",
        seg,
        re.DOTALL,
    )
    if m:
...
def extract_body(seg):
    ...
    inner = re.sub(
        r'<span class=[\'"]ipsType_reset ipsType_medium ipsType_light[\'"][^>]*>.*?</span>',
        "",
        inner,
        flags=re.DOTALL,
    )
...
def strip_quote_blocks(body):
    ...
    def repl(m):
        ...
        inner = re.sub(r"<cite[^>]*>.*?</cite>", "", inner, flags=re.DOTALL)
    ...
    body = re.sub(r"<blockquote[^>]*>.*?</blockquote>", repl, body, flags=re.DOTALL)
    return body
...
def extract_images(body):
    ...
    body = re.sub(
        r'<a\s+class=["\']ipsAttachLink[^"\']*["\'][^>]*?\shref=[\'"]([^\'"]+)[\'"][^>]*>(.*?)</a>',
        attach_repl,
        body,
        flags=re.DOTALL,
    )
...
if __name__ == "__main__":
    for p in sys.argv[1:]:
        ...
        for post in posts:
            ...
            print(
                f"  {mark}  post#{post['postnum']} comment={post['commentid']} date={post['date']} edited={bool(post['edited'])} bodylen={len(post['body'])}"
            )
