...
with open("/tmp/allimgs.txt") as _fh:
    IMAGES = [l.strip() for l in _fh if l.strip()]
...
for d in dirs:
    ...
    if not os.path.exists(out):
        q = (
            ...
            + "&matchType=prefix&output=json&fl=timestamp,original,statuscode,mimetype&filter=statuscode:200&limit=20000"
        )
        r = subprocess.run(
            ["curl", "-s", "--max-time", "60", q],
            capture_output=True,
            text=True,
            check=False,
        )
...
for u in IMAGES:
    ...
    results[u] = {
        "base": base,
        "exact": matches,
        "thumb_variants": sorted({(c, t) for c, t, o in cands}),
        "thumb_urls": sorted({o for c, t, o in cands}),
    }
    if matches:
