...
with open(os.path.join(BASE, "richard_posts.json")) as fh:
    posts = json.load(fh)
with open(os.path.join(BASE, "image_archive_map.json")) as fh:
    imgmap = json.load(fh)
...
def resolve_image(url):
    ...
    if info["thumb_urls"]:
        ...
        for cand, ts in info["thumb_variants"]:
            if cand in tu:
                return (
                    f"https://web.archive.org/web/{ts}id_/{tu}",
                    f"wayback-thumb-{ts}",
                )
        return (
            f"https://web.archive.org/web/{info['thumb_variants'][0][1]}id_/{tu}",
            "wayback-thumb",
        )
    return url, None
...
def rewrite_body(body):
    ...
    for line in body.split("\n"):
        m = re.match(r"!\[([^\]]*)\]\(([^)]+)\)$", line)
        if m:
            src, _tag = resolve_image(m.group(2))
...
lines = []
lines.append(
    '# Trireme "Olympias" by Richard Braithwaite — Recovered Pre-Hack Build Log (Topic 21958)'
)
lines.append("")
lines.append(
    "Source: [Model Ship World — archived topic 21958](https://web.archive.org/web/20250427010555/https://modelshipworld.com/topic/21958-trireme-olympias-by-richard-braithwaite/), recovered from the Internet Archive Wayback Machine after the site was hacked."
)
lines.append("")
lines.append(
    'This is the **pre-hack build log** (topic 21958, "Trireme Olympias by Richard Braithwaite", started September 2019). It is **separate from** the newer re-posted log (topic 424) which is recorded in `trireme-olympias-build-log-richard-braithwaite.md`.'
)
lines.append("")
lines.append("### Recovery summary")
lines.append("")
lines.append(
    "- The archived thread had **9 pages** (per the pagination of the latest page-1 snapshot)."
)
lines.append(
    "- Wayback captured **pages 1–5** (posts 1–126 of the thread). **Pages 6–9 were never archived** and their full text is lost."
)
lines.append(
    "- **49 of Richard's posts** (Sep 2019 – May 2022) were recovered in full from pages 1–5, in chronological order below."
)
lines.append(
    "- **8 additional post snippets** (June–July 2025, i.e. the missing pages) were recovered from Richard's profile activity stream and are listed in a separate section below as *fragments*."
)
lines.append(
    "- Images: where an image was archived by Wayback, the link points to the archived copy; otherwise the original (now-broken) URL is kept."
)
lines.append("")
lines.append("---")
lines.append("")
...
for i, p in enumerate(order, 1):
    lines.append(
        f"## Post {i} — {p['author']} — #{p['postnum']} in thread ({fmt_date(p['date'])})"
    )
    lines.append("")
...
lines.append(
    "## Fragments from the un-archived pages 6–9 (via Richard's profile activity stream)"
)
lines.append("")
lines.append(
    "These are the only recoverable text from the final pages of the old thread (June–July 2025). They were captured as snippet excerpts on Richard's MSW profile page, not as full posts."
)
lines.append("")
