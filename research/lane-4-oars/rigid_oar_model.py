...
RIGS = {
    ...
    # the cos(cant) flow correction, 16)
    "Olympias": {
        "lin": 0.957,
        "lout": 2.696,
        "blade": 0.55,
...
def rigid_stroke(V, rig, r_spm, t_drive=None, n_pts=600):
    ...
    for i in range(N):
        ...
        w = -omega_mean
        _ux, _uy = math.sin(C), math.cos(C)
        cf = math.cos(math.radians(rig.get("cant", 0.0)))  # 16
        ...
        # form, consistent with ll/blade.py (16)
        vn = V * nx + l_cp * w
...
def main():
    ...
    print("=" * 78)
    print(f"Blade C_N = {CN} (flat plate; = Macon C_Dmax ~1.85, C&G 2007b), rho={RHO}")
    print(
        f"{'rig':9} {'V kt':>5} {'spm':>5} {'thrust/oar':>11} {'eff':>6} "
        f"{'mean Fh':>8} {'peak Fb':>8}  Shaw bulk E / P"
    )
    for name, rig, Vkt, r_spm in [
        ("Olympias", "Olympias", 7.2, 28.8),
        ...
        print(
            f"{name:9} {Vkt:5.1f} {r_spm:5.1f} {s['mean_thrust']:11.1f} "
    ...
    for name, rig, Vkt, r_spm in [
        ...
        print(
            f"  {name:9} base area {base_area:.3f} m^2 -> required ~{need:.3f} m^2 "
