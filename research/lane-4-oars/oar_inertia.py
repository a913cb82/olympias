...
DRIVES = [
    ("Olympias", 28.8, 0.430, 48.1),  # 7.2 kt
    ("Olympias", 36.0, 0.392, 48.1),  # 8.2 kt
    ("MarkIIb", 46.3, 0.472, 55.6),
]  # 9.7 kt (sprint)
...
def main():
    ...
    hdr = (
        f"{'oar':>3} {'type':>11} {'W kgf':>6} {'k2 m2':>6} {'COG m':>6} "
    ...
    for r in rows:
        ...
        print(
            f"{r['oar']:>3} {r['typ']:>11} {r['Wkgf']:6.2f} "
    ...
    for key in ("spruce", "fir-zygian", "fir-thranite"):
        ...
        print(f"  {key:>12} {len(rs):>2} {w:>9.1f} {mit:>14.1f} {mit / LIN**2:>12.1f}")
    ...
    I = {
        k: sum(r["mit"] for r in rs) / len(rs)
        for k, rs in [("spruce", spr), ("zygian", zyg), ("thranite", thr)]
    }
    hdr2 = f"{'case':>22} {'rate':>5} {'omega':>6} | " + " | ".join(
        f"{k:>18}" for k in ("spruce", "zygian", "thranite")
    )
    print(hdr2)
    print("-" * 88)
    for name, r_spm, t_drive, sweep_deg in DRIVES:
        ...
        for k in ("spruce", "zygian", "thranite"):
            cells.append(
                " ".join(
                    f"{catch_spike(I[k], omega, tr):5.0f}" for tr in (0.10, 0.15, 0.20)
                )
            )
        print(f"{name:>22} {r_spm:5.1f} {omega:5.2f} | " + " | ".join(cells))
    print("   (t_rise 0.10 / 0.15 / 0.20 s;  mean drive handle force is ~210-225 N)")
