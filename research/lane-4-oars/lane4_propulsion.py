...
def sprint_validation():
    ...
    print(
        f"(a) 116-rower sprint: P = {P_cal:5.0f} N  ({P_cal / 4.448:4.1f} lbf)   [Shaw: 288 N / 64.7 lbf]"
    )
    print(
        f"    implied mean-pull law constant k = P/r = {P_cal / r_cal:.3f}   [Shaw uses 7.43]"
    )
    ...
    print(
        f"    power = {W4:7.0f} W  ->  {V4:.3f} m/s = {V4 / KT:.2f} kts   [Shaw: 18152 W -> 8.32 kts]"
    )
    print(
        f"    measured: 8.2-8.3 kts.  PREDICTION MATCHES EXPERIMENT: {abs(V4 / KT - 8.32) < 0.05}"
    )
...
def table_97():
    ...
    print("=" * 68)
    print(
        f"{'design':6} {'V kt':>6} {'W W':>7} {'L m':>5} {'r spm':>7} {'P N':>6} "
    ...
    for hull, L, Vkt in [
        ...
        print(
            f"{hull:6} {Vkt:6.1f} {W:7.0f} {L:5.2f} {r:7.1f} {P:6.0f} {P / 4.448:7.1f} "
            f"{P * r:9.0f} {r * r:6.0f}"
        )
...
def table_ch7():
    ...
    print(
        "    gross power/man = P*L*r/60 + oar_absorbed(r)   [handle power + oar losses]"
    )
    print("=" * 68)
    print(
        f"{'V kt':>6} {'W W':>7} {'r spm':>7} {'P N':>6} {'handle':>8} {'oar_abs':>8} "
        f"{'gross W':>8}  vs Shaw r / gross"
    )
    for Vkt, r_shaw, Wman_shaw in [
        (7.0, 25.5, 115),
        (7.5, 28.8, 145),
        (8.0, 32.3, 180),
    ]:
        ...
        print(
            f"{Vkt:6.1f} {W:7.0f} {r:7.1f} {P:6.0f} {handle:8.1f} {absorbed:8.1f} "
