...
| `lane-2-waves` | Wave environment | `waves.md` (Shaw Tables 8.1–8.4 — Carter's growth equations + the wind tables) |
| `lane-3-hull` | Hull form & resistance | `hull-form.md` (the parametric reconstruction + the offsets availability/Eliav & Helfman; the KM stability caveat on record), `hull-form-summary.csv`, `hull_form.py`, `braithwaite-workbook.md` (the real Lines-Plan offsets + hydrostatics + the weight breakdown — from the author's design tool; lightship 25.75 t confirms the 25.798 t anchor) |
| `lane-4-oars` | Oar rig, power, blade physics | `rig-and-oars.md` (geometry, Table 3.1/3.2 decoded, the rigid-oar flat-plate layer), `propulsion-models.md`, `lane4_propulsion.py` (the per-rower power law P = 7.43·r), `rigid_oar_model.py`, `oar_inertia.py` (the catch-flip spike — the oar's snap at the start of the stroke), `braithwaite-workbook.md` (the trials thrust law 81 N→0 @ 18 kt — agrees with our zero-speed equilibrium; the trials resistance fit; the 9.95 kt no-head-room bound) |
| `lane-5-manoeuvre` | Turning | `manoeuvre.md` (the reference model + the Excel/Kempf notes + the F/G re-run ≤ 7 % + the CLR rotation), `manoeuvre_model.py`, `fg_turns_rerun.py`, `clr_rotation.py`, `crossflow.py` (the Ω audit), `braithwaite-workbook.md` (the independent 3-DOF model: CGH derivatives, the cross-flow yaw damper CN 0.4/0.8 `[?]`, Iz = m(L/3)², the Hoerner rudder + the trials parasitic drag) |
| `lane-6-validation` | Cross-cutting validation | `validation.md` (the trial data + the bulk-vs-trials mapping + the sensitivity pass), `uncertainties-register.md` (every `[?]` item, its impact), `validation_table.py`. The simulator's acceptance record is `../simulation/docs/VALIDATION.md` |
